...
function connectionPort (connection) {
	return typeof connection.info.port === "number" ? connection.info.port :
...
mummy.embalm = function embalm (server, browser) {
	...
	function start () {
		if (!server._start) {
			...
			server._start = Q.ninvoke(server, "start");
		}
	...
	browser.http = function (options, callback) {
		...
		.then(function () {
			...
			if (connection) {
				connection.inject(prepareRequest(options), deferred.resolve.bind(deferred));
			}
			else {
